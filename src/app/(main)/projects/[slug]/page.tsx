...
export default async function ProjectDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    ...
    >
      <AnalyticsTracker type="project" slug={slug} title={project.title} />
            ...
            <Flex gap="8" wrap vertical="center">
              ...
              {project.tags.map((tag) => (
                <Flex key={tag} paddingX="8" paddingY="4" radius="m" background="brand-alpha-weak">
                  <Text variant="code-default-xs" onBackground="brand-strong">
                    #{tag}
                  </Text>
                </Flex>
              ))}
          ...
          <Scroller fillWidth gap="16" paddingBottom="12">
            ...
            {project.screenshots &&
              project.screenshots.map((ss, i) => (
                <div
                  ...
                  className="relative aspect-[16/9] rounded-xl overflow-hidden border border-neutral-alpha-weak shadow-md bg-surface"
                >
            ...
            {(!project.screenshots || project.screenshots.length < 3) &&
              [1, 2, 3].map((_, i) => (
                <Flex
                  key={i}
                  style={{ minWidth: "400px", aspectRatio: "16/9" }}
                  background="surface"
                  border="neutral-alpha-weak"
                  radius="l"
                  center
